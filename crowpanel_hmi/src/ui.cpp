...
static void create_dashboard_tab(lv_obj_t* parent) {
    ...
    // Top row: Water sensors (3 cards) + connection status
    // Row 1: Flow Rate | Water Temp | Rain Sensor | Room Temp | Room Humidity
...
static void create_actions_tab(lv_obj_t* parent) {
    ...
    static const lv_color_t card_colors[] = {
        COLOR_WARN, COLOR_RAIN, COLOR_WATER, COLOR_TEMP
    };
    ...
    for (int i = 0; i < ui_data.num_actions && i < MAX_ACTIONS; i++) {
        ...
        lv_obj_add_style(card, &style_card, 0);
        lv_obj_set_style_border_color(card, card_colors[a->type < 4 ? a->type : 0], 0);
        lv_obj_clear_flag(card, LV_OBJ_FLAG_SCROLLABLE);
...
// ============================================================
void ui_set_sensor_data(float flow, float temp, float totalLiters, bool isRaining, int rainAnalog) {
        ...
        switch (a->type) {
            ...
            case ACTION_RAIN_AUTO: {
                ...
                if (raining && !a->triggered && !override) {
                    ...
                    a->triggered = false;
                    if (!override) relay_command(a->target_relay, 0);
