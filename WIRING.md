...
|--------|-------------|-------|
| ADS1115 (16-bit ADC) | 0x49 | ADDR pin → VDD |
...
- SDA → GPIO 8
- SCL → GPIO 9
- ADDR → VDD (sets I2C address to 0x49)
- A0 → Wind speed sensor signal wire

> No external pull-ups needed on I2C — the ADS1115 module has built-in pull-ups.

### Wind Speed Sensor (0-5V Analog, via ADS1115 A0)
- Red → 12V (external power supply, check sensor spec)
- Black → GND (common with ESP32-C3 GND)
- Signal (Yellow/Blue) → ADS1115 A0 input

**Output mapping:** 1-5V signal (NOT 0-5V)
- **< 1V** = Sensor error / disconnected
- **1V** = 0 m/s
- **5V** = 60 m/s
- Formula: `windSpeed = ((voltage - 1.0) / 4.0) * 60.0` m/s

> The ADS1115 is used because the ESP32-C3 built-in ADC is 3.3V max and
> would need a voltage divider. The ADS1115 at GAIN_TWOTHIRDS covers ±6.144V,
> easily handling the 0-5V sensor output with 16-bit precision.
...
### Dashboard Tab
- Shows all sensor values in card format (6 cards)
- Water flow, water temp, rain, wind speed, room temp, humidity
...
### Actions Tab
- 5 sensor-based automation rules with editable target relay and threshold
- **Pipe Leak Alarm:** flow > threshold → turn ON alarm relay
- **Rain Auto-Close:** rain detected → close shed relay, auto-reverts when dry
- **No Water Alarm:** scheduled relay running 30s+ with no flow detected
- **Freezing Temperature:** water temp ≤ threshold → emergency dump relay
- **High Wind Speed:** wind > threshold → protective relay, clears on 15-min avg
- Multi-action relay sharing: relay stays ON as long as ANY action targeting it is triggered
- Manual override from CrowPanel, Wroom buttons, or RainMaker app
- Alarms forwarded to RainMaker cloud with per-type notifications
...
| Wind speed always 0 | Check ADS1115 at 0x49 (ADDR→VDD), verify sensor signal on A0, check 12V power |
| Wind reading < 1V | Sensor error or disconnected — code reports 0 m/s for voltages below 1V |
